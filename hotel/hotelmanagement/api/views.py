...
class GuestBookingListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if arrive_date is not None and depart_date is not None:
            queryset1 = Guest.objects.all().filter(Q(booking__arrive_date__range=[arrive_date, depart_date]) and Q(booking__depart_date__range=[arrive_date, depart_date]))
            queryset2 = Guest.objects.all().filter(booking__arrive_date__lte=arrive_date).filter(booking__depart_date__gte=depart_date)
            queryset3 = Guest.objects.all().filter(Q(booking__arrive_date__lt=arrive_date) and Q(booking__arrive_date__lt= depart_date)).filter(booking__depart_date__gte=depart_date)
...
class GuestBookingFOListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if session_date is not None:
            ...
            queryset = Guest.objects.all().filter(booking__arrive_date__lte=session_date).filter(booking__depart_date__gte=session_date)
        return queryset